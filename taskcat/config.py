...
class Test:
    def __init__(self, template_file: Path, parameter_input: Path = None, parameters: dict = None, regions: set = None,
        ...
        self.parameters: Dict[str, int, bool] = self._params_from_file() if parameter_input else {}
        if parameters:
    ...
    def _guess_path(self, path):
        ...
        if not abs_path:
            raise TaskCatException(f"Cannot find {path} with project root {self._project_root}")
        return abs_path

    def _legacy_path_prefix(self, path):
        abs_path = absolute_path(self._project_root / "templates" / path)
        if abs_path:
            LOG.warning(f"found path with deprecated relative path, support for this will be removed in future "
                        f"versions, please update {path} to templates/{path}")
            return abs_path
        abs_path = absolute_path(self._project_root / "ci" / path)
        if abs_path:
            LOG.warning(f"found path with deprecated relative path, support for this will be removed in future "
                        f"versions, please update {path} to ci/{path}")
            return abs_path
    ...
    def _validate_params(self, params):
        try:
            ...
            try:
                Config.validate(params, 'legacy_parameters')
                LOG.warning("%s parameters are in a format that will be deprecated in the next version of taskcat",
...
class Config:
    ...
    def _propogate_regions(self, client_factory_instance):
        default_region = client_factory_instance.get_default_region(None, None, None, None)
        for test in self.tests.keys():
            if not self.tests[test].regions and not default_region and not self.regions:
                raise TaskCatException(f"unable to define region for test {test}, you must define regions or set a "
                                       f"default region in the aws cli")
            elif not self.tests[test].regions:
                self.tests[test].regions = self.regions if self.regions else [default_region]
    ...
    def validate(instance, schema_name):
        ...
        validate(instance_copy, schema, resolver=RefResolver(str(schema_path.as_uri()) + '/', None))
    ...
    def _process_project_config(self):
        ...
        if 'tests' in instance.keys():
            ...
            for test in instance['tests'].keys():
                tests[test] = Test.from_dict(instance['tests'][test], project_root=self.project_root)
    ...
    def _process_template_config(self):
        ...
        try:
            ...
            raise TaskCatException(f"failed adding config from template file {str(self.template_path)} "
    ...
    def _add_template_path(self, template_config):
        if 'tests' in template_config.keys():
            for test in template_config['tests'].keys():
                ...
                template_config['tests'][test] = Test.from_dict(template_config['tests'][test],
    ...
    def _to_tests(args: dict):
        if 'template_file' in args.keys() or 'parameter_input' in args.keys() or 'regions' in args.keys():
            ...
            regions = set(args['regions'].split(',')) if 'regions' in args.keys() else set()
            test = Test(template_file=template_file, parameter_input=parameter_input, regions=regions,
                        project_root=self.project_root)
